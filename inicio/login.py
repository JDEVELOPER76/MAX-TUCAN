...
class LoginApp:
    ...
    # ---------- UI ----------
    def setup_ui(self):
        # Detectar modo web vs desktop/exe
        img_src = "/tucan.jpg" if self.page.web else resource_path("assets/tucan.jpg")
...
# ---------- LOGIN ----------
    ...
    def validar_login(self, e):
        ...
        if admin_data and admin_data.get("password") == contraseña:
            ...
            db.registrar_auditoria(
                usuario=usuario,
                tipo="login",
                descripcion="Inicio de sesión como Administrador",
                detalles=f"Acceso al panel administrativo"
            )
            self.navegar_a_admin(usuario)
            return
        ...
        if emp_data and emp_data.get("password") == contraseña:
            ...
            db.registrar_auditoria(
                usuario=usuario,
                tipo="login",
                descripcion="Inicio de sesión como Empleado",
                detalles=f"Acceso al sistema de ventas"
            )
    ...
    def navegar_a_admin(self, usuario="Administrador"):
        self.page.clean()
        AdminPanel(self.page, usuario)
        self.page.update()
