...
class ProductosWindow:
    ...
    def _build_filtros(self):
        ...
        self.dropdown_filtro = ft.Dropdown(
            label="Filtrar por...",
            options=[
                ft.dropdown.Option("Todos"),
                ft.dropdown.Option("Stock Bajo"),
                ft.dropdown.Option("Nuevos Hoy"),
                ft.dropdown.Option("Destacados"),
        ...
        try:
            with get_conn() as conn:
                ...
                if self.filtro_activo and self.filtro_activo != "Todos":
                    if self.filtro_activo == "Stock Bajo":
                        query += " AND p.stock_actual <= p.stock_minimo"
                    elif self.filtro_activo == "Nuevos Hoy":
                        hoy = datetime.now().strftime("%Y-%m-%d")
                        query += " AND DATE(p.creado_en) = ?"
                        params.append(hoy)
                    elif self.filtro_activo == "Destacados":
                        query += " AND p.destacado = 1"
                    elif self.filtro_activo == "Normal":
    ...
    def _buscar_productos(self, e):
        ...
        self.cargar_productos(self.termino_busqueda, self.filtro_activo)

    def _aplicar_filtro(self, e):
        """Aplica el filtro seleccionado"""
        if self.dropdown_filtro.value:
            self.filtro_activo = self.dropdown_filtro.value
            self.cargar_productos(self.termino_busqueda, self.filtro_activo)

    def _limpiar_filtros(self, e):
        """Limpia todos los filtros y vuelve a defecto"""
        self.termino_busqueda = ""
        self.filtro_activo = "Todos"
        self.dropdown_filtro.value = "Todos"
        self.page.update()
        self.cargar_productos()

    def _toggle_destacado(self, producto):
        """Marca o desmarca un producto como destacado"""
        try:
            nuevo_estado = 1 - producto["destacado"]  # Invertir estado
            with get_conn() as conn:
                conn.execute(
            ...
            producto["destacado"] = nuevo_estado
            
            # Recargar productos y estadísticas
            self.cargar_productos(self.termino_busqueda, self.filtro_activo)
            self._cargar_estadisticas()
            self._mostrar_mensaje(
                "Producto marcado como destacado" if nuevo_estado else "Producto desmarcado de destacados"
            )
        except Exception as e:
            print(f"Error al cambiar estado de destacado: {e}")
            self._mostrar_mensaje(f"Error: {e}")
    ...
    def _ver_detalles(self, producto):
        ...
        # Construir tipos de venta activos en línea
        tipos_venta = []
        if producto["venta_normal_activa"]:
            tipos_venta.append("Normal")
        if producto["venta_mayoreo_activa"]:
            tipos_venta.append("Mayoreo")
        if producto["venta_promocion_activa"]:
            tipos_venta.append("Promoción")
        
        tipos_venta_str = ", ".join(tipos_venta) if tipos_venta else "Ninguno"
        ...
        detalles = ft.AlertDialog(
            ...
            content=ft.Column([
                ...
                ft.Text(f"Stock: {producto['stock_actual']} (Mín: {producto['stock_minimo']})"),
                ...
                ft.Text(f"Tipos de Venta: {tipos_venta_str}"),
            ], tight=True),
    ...
    def _construir_dialog(self):
        ...
        content = ft.Container(
            content=ft.Column([
                ...
                # Stocks
                ft.ResponsiveRow([
                    ft.Container(self.stock_actual, col=6, padding=5),
                    ft.Container(self.stock_minimo, col=6, padding=5),
                ]),
                ...
                if self.es_edicion:
                    ...
                    conn.execute("""
                        INSERT INTO productos (
                            codigo_barras, nombre, precio_compra,
                            precio_venta_normal, precio_venta_mayoreo, precio_venta_promocion,
                            stock_actual, stock_minimo, iva_porcentaje,
                            venta_normal_activa, venta_mayoreo_activa, venta_promocion_activa,
