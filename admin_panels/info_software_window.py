...
class InfoSoftwareWindow:
    def __init__(self, page: ft.Page, admin_panel):
        ...
        self.version_file = Path(".version/version.zxc")
    ...
    def _leer_version(self):
        """Lee la versión desde el archivo version.zxc"""
        try:
            if self.version_file.exists():
                return self.version_file.read_text(encoding='utf-8').strip()
            else:
                # Si no existe, crear con versión por defecto
                self.version_file.write_text("1.0.0", encoding='utf-8')
                return "1.0.0"
        except Exception as e:
            print(f"Error al leer version.zxc: {e}")
            return "1.0.0"
    ...
    def build_ui(self):
        ...
        info = [
            ...
            ("Versión actual", self._leer_version()),
    ...
    def _verificar_actualizacion(self, e):
        ...
        def check_update_thread():
            ...
            version_local = self._leer_version()
    ...
    def _abrir_explorador(self, e):
        self.file_picker.pick_files(
            ...
            allow_multiple=True,
            allowed_extensions=["zip", "db", "json"]
        )
    ...
    def _file_picker_result(self, e: ft.FilePickerResultEvent):
        ...
        for f in e.files:
            ...
            if nombre_archivo.endswith(".zip"):
                try:
                    with zipfile.ZipFile(f.path, "r") as zf:
                        for info in zf.infolist():
                            base_name = Path(info.filename).name.lower()
                            if not base_name:
                                continue
                            for _, destino in self.archivos_requeridos.items():
                                if base_name == destino.name.lower():
                                    with zf.open(info, "r") as src, open(destino, "wb") as dst:
