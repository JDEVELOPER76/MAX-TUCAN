...
class MenuVentas:
    ...
    def cargar_productos_db(self):
        """Carga productos desde la base de datos (todos los activos, incluso con stock 0)."""
        try:
            ...
            # ✅ Traer todos los productos activos, sin excluir stock 0
            cursor.execute("""
                SELECT 
            ...
            for producto in productos_db:
                # Obtener el precio adecuado según el tipo de venta
            ...
            if not self.productos:
                print("ℹ️ No se encontraron productos activos en la base de datos.")
                self.productos = []
                self.total_productos = 0
    ...
    def cargar_productos_ui(self, productos):
        """Carga hasta 60 productos visibles, evitando errores de actualización."""
        ...
        # No actualizar si el grid aún no está en la página
        if hasattr(self.lista_productos, "_Control__uid") and self.lista_productos._Control__uid:
            self.lista_productos.expand = True
            self.lista_productos.height = None
            self.lista_productos.auto_scroll = True
            self.lista_productos.update()


    def filtrar_productos(self, e):
        """Filtra productos por nombre o código, evitando conflictos de actualización."""
        try:
            ...
            filtrados = [
                ...
                or (p.get("codigo_barras") and texto in p["codigo_barras"].lower())
            ]

            self.cargar_productos_ui(filtrados)
        except Exception as ex:
            print(f"⚠️ Error en filtrar_productos: {ex}")
